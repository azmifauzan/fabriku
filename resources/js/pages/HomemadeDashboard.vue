...
import { AlertTriangle, Package, ShoppingCart, TrendingUp, Factory, ShieldAlert, Sparkles } from 'lucide-vue-next';
...
<template>
    <AppLayout>
        ...
        <div class="px-6 py-6 bg-gray-50/50 min-h-screen dark:bg-gray-900/50">
            <div class="mx-auto max-w-7xl space-y-6">
                ...
                <div class="flex flex-col gap-2 md:flex-row md:items-center md:justify-between">
                    <div>
                        <h1 class="text-2xl font-bold text-gray-900 dark:text-gray-100 flex items-center gap-2">
                            ...
                            <Sparkles :size="20" class="text-indigo-500 animate-pulse" />
                        </h1>
                ...
                <div class="grid grid-cols-2 gap-4 sm:grid-cols-4">
                    <!-- Revenue Today -->
                    <div class="rounded-2xl border border-gray-100 bg-white p-5 shadow-sm hover:shadow-md transition-shadow dark:border-gray-800 dark:bg-gray-800">
                        <div class="flex items-center justify-between">
                            <p class="text-xs font-semibold text-gray-500 dark:text-gray-400">Penjualan Hari Ini</p>
                            <div class="p-2 bg-green-50 rounded-xl dark:bg-green-950/30">
                        ...
                        <p class="text-xs text-gray-500 dark:text-gray-400 mt-1 font-medium">{{ stats.sales_today_count }} transaksi</p>
                    </div>
                    ...
                    <div class="rounded-2xl border border-gray-100 bg-white p-5 shadow-sm hover:shadow-md transition-shadow dark:border-gray-800 dark:bg-gray-800">
                        <div class="flex items-center justify-between">
                            ...
                            <div class="p-2 bg-indigo-50 rounded-xl dark:bg-indigo-950/30">
                        ...
                        <p class="text-xs text-gray-500 dark:text-gray-400 mt-1 font-medium">{{ stats.sales_month_count }} transaksi</p>
                    </div>
                    ...
                    <div class="rounded-2xl border border-gray-100 bg-white p-5 shadow-sm hover:shadow-md transition-shadow dark:border-gray-800 dark:bg-gray-800">
                        <div class="flex items-center justify-between">
                            ...
                            <div class="p-2 bg-blue-50 rounded-xl dark:bg-blue-950/30">
                        ...
                        <p class="text-xs text-gray-500 dark:text-gray-400 mt-1 font-medium">{{ stats.total_inventory_items }} SKU produk jadi</p>
                    </div>
                    ...
                    <div class="rounded-2xl border border-gray-100 bg-white p-5 shadow-sm hover:shadow-md transition-shadow dark:border-gray-800 dark:bg-gray-800">
                        <div class="flex items-center justify-between">
                            ...
                            <div class="p-2 bg-purple-50 rounded-xl dark:bg-purple-950/30">
                                <Factory :size="16" class="text-purple-600 dark:text-purple-400" />
                            </div>
                        </div>
                        <p class="mt-3 text-xl font-bold text-gray-900 dark:text-gray-100">{{ stats.production_today }} <span class="text-xs font-normal text-gray-400">unit</span></p>
                        <p class="text-xs text-gray-500 dark:text-gray-400 mt-1 font-medium">Dari catatan produksi harian</p>
                    </div>
                </div>
                ...
                <div class="grid grid-cols-1 gap-6 lg:grid-cols-2">
                    ...
                    <div class="rounded-2xl border border-gray-100 bg-white p-6 shadow-sm dark:border-gray-800 dark:bg-gray-800">
                        ...
                        <div v-if="salesTrend.length === 0" class="py-12 text-center text-sm text-gray-400">
                        ...
                        <div v-else class="space-y-3">
                            <div v-for="day in salesTrend" :key="day.date" class="flex items-center gap-3">
                                ...
                                <span class="w-12 text-right text-xs text-gray-400 font-medium">{{ day.count }}x</span>
                            </div>
                        </div>
                    </div>
                    ...
                    <div class="rounded-2xl border border-gray-100 bg-white p-6 shadow-sm dark:border-gray-800 dark:bg-gray-800">
                        ...
                        <div v-if="topProducts.length === 0" class="py-12 text-center text-sm text-gray-400">
                            Belum ada data penjualan
                        </div>
                        <div v-else class="space-y-4">
                            <div v-for="(prod, idx) in topProducts" :key="prod.sku" class="flex items-center gap-3">
                                <span class="flex h-7 w-7 items-center justify-center rounded-full bg-indigo-50 text-xs font-bold text-indigo-600 dark:bg-indigo-900/30 dark:text-indigo-400 shrink-0">
                                    {{ idx + 1 }}
                                </span>
                    ...
                    <div class="rounded-2xl border border-gray-100 bg-white p-6 shadow-sm dark:border-gray-800 dark:bg-gray-800">
                        <div class="mb-5 flex items-center justify-between">
                            <h2 class="text-sm font-bold text-gray-800 dark:text-gray-200 flex items-center gap-2">
                                ...
                                <span v-if="stats.low_stock_materials_count > 0" class="flex h-5 w-5 items-center justify-center rounded-full bg-red-100 text-[10px] font-bold text-red-600 dark:bg-red-950/30 dark:text-red-400">
                                    {{ stats.low_stock_materials_count }}
                                </span>
                            </h2>
                            <Link href="/materials" class="text-xs font-semibold text-indigo-600 hover:underline dark:text-indigo-400">Lihat semua</Link>
                        </div>
                        <div v-if="lowStockMaterials.length === 0" class="py-12 text-center text-sm text-gray-400">
                            Semua stok bahan baku aman
                        </div>
                        <div v-else class="space-y-3">
                            <div v-for="mat in lowStockMaterials" :key="mat.id" class="flex items-center justify-between gap-3 rounded-xl border border-red-50 bg-red-50/20 px-4 py-3 dark:border-red-950/20 dark:bg-red-950/5">
                                <div class="min-w-0">
                    ...
                    <div class="rounded-2xl border border-gray-100 bg-white p-6 shadow-sm dark:border-gray-800 dark:bg-gray-800">
                        <div class="mb-5 flex items-center justify-between">
                            <h2 class="text-sm font-bold text-gray-800 dark:text-gray-200 flex items-center gap-2">
                                ...
                                <span v-if="stats.low_stock_count > 0" class="flex h-5 w-5 items-center justify-center rounded-full bg-orange-100 text-[10px] font-bold text-orange-600 dark:bg-orange-950/30 dark:text-orange-400">
                                    {{ stats.low_stock_count }}
                                </span>
                            </h2>
                            <Link href="/inventory/items" class="text-xs font-semibold text-indigo-600 hover:underline dark:text-indigo-400">Lihat semua</Link>
                        </div>
                        <div v-if="lowStockItems.length === 0" class="py-12 text-center text-sm text-gray-400">
                            Semua stok produk jadi aman
                        </div>
                        <div v-else class="space-y-3">
                            <div v-for="item in lowStockItems" :key="item.id" class="flex items-center justify-between gap-3 rounded-xl border border-orange-50 bg-orange-50/20 px-4 py-3 dark:border-orange-950/20 dark:bg-orange-950/5">
                                <div class="min-w-0">
                    ...
                    <div class="rounded-2xl border border-gray-100 bg-white p-6 shadow-sm lg:col-span-2 dark:border-gray-800 dark:bg-gray-800">
                        <div class="mb-5 flex items-center justify-between">
                            ...
                            <Link href="/simple-production" class="text-xs font-semibold text-indigo-600 hover:underline dark:text-indigo-400">Lihat semua</Link>
                        </div>
                        ...
                        <div v-else class="overflow-x-auto">
                            <table class="w-full text-left border-collapse">
                                <thead>
                                    <tr class="border-b border-gray-100 text-xs font-bold uppercase tracking-wider text-gray-400 dark:border-gray-700">
                                        <th class="pb-3 pr-4">Tanggal</th>
                                        <th class="pb-3 pr-4">Batch ID</th>
                                        <th class="pb-3 pr-4">Produk</th>
                                        <th class="pb-3 text-right">Jumlah</th>
                                    </tr>
                                </thead>
                                <tbody>
                                    <tr v-for="prod in recentProductions" :key="prod.batch_id" class="border-b border-gray-50/50 hover:bg-gray-50/20 text-sm text-gray-700 dark:border-gray-750 dark:text-gray-300 dark:hover:bg-gray-800/10">
                                        <td class="py-3.5 pr-4 font-medium">
                                            {{ new Date(prod.created_at).toLocaleDateString('id-ID', { day: '2-digit', month: 'short', year: 'numeric' }) }}
                                        </td>
                                        <td class="py-3.5 pr-4 font-mono text-xs text-gray-400">
                                        ...
                                        <td class="py-3.5 text-right font-bold text-indigo-600 dark:text-indigo-400">
                                            +{{ prod.total_qty }}
                                        </td>
                                    </tr>
                                </tbody>

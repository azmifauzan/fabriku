            ...
            <div class="mx-auto max-w-4xl space-y-6">
                ...
                <div class="flex items-center gap-3">
                    ...
                    <div>
                        ...
                        <p class="text-xs text-gray-500 dark:text-gray-400">
                            Informasi lengkap hasil produksi dan bahan baku yang digunakan dalam batch ini
                        </p>
                    </div>
                </div>
                ...
                <div class="grid grid-cols-1 gap-6 md:grid-cols-3">
                    ...
                    <div class="space-y-6 md:col-span-1">
                        <div class="space-y-4 rounded-2xl border border-gray-100 bg-white p-5 shadow-sm dark:border-gray-800 dark:bg-gray-800">
                            <h3 class="text-xs font-bold tracking-wider text-gray-400 uppercase">Informasi Transaksi</h3>

                            <!-- Batch ID -->
                            <div>
                                <span class="text-xs font-semibold text-gray-400">Batch ID</span>
                                <p class="mt-0.5 font-mono text-xs font-bold text-indigo-600 dark:text-indigo-400">{{ summary.batch_id }}</p>
                            </div>
                            ...
                            <div class="flex items-center gap-2">
                                ...
                                <div>
                                    <span class="block text-xs font-semibold text-gray-400">Tanggal Produksi</span>
                            ...
                            <div class="flex items-center gap-2">
                                ...
                                <div>
                                    <span class="block text-xs font-semibold text-gray-400">Dicatat Oleh</span>
                                    <span class="text-sm font-bold text-gray-800 dark:text-gray-200">{{ summary.adjusted_by?.name || '-' }}</span>
                                </div>
                            </div>

                            <!-- Notes -->
                            <div v-if="summary.notes" class="border-t border-gray-100 pt-3 dark:border-gray-700">
                                <span class="text-xs font-semibold text-gray-400">Catatan</span>
                                <p class="mt-1 rounded-lg bg-gray-50 p-2.5 text-xs leading-relaxed text-gray-600 dark:bg-gray-900 dark:text-gray-400">
                                    {{ summary.notes }}
                                </p>
                            </div>
                        </div>
                    ...
                    <div class="space-y-6 md:col-span-2">
                        ...
                        <div class="space-y-4 rounded-2xl border border-gray-100 bg-white p-6 shadow-sm dark:border-gray-800 dark:bg-gray-800">
                            ...
                            <div v-for="line in lines" :key="line.id" class="flex flex-col gap-4 py-2 sm:flex-row sm:items-center sm:justify-between">
                                <div class="min-w-0" v-if="line.inventory_item">
                                    <p class="text-md font-bold text-gray-800 dark:text-gray-200">{{ line.inventory_item.product_name }}</p>
                                    <p class="text-gray-450 mt-1 text-xs font-semibold">SKU: {{ line.inventory_item.sku }}</p>
                                </div>
                                <div class="flex shrink-0 gap-6 sm:text-right">
                                    ...
                                    <div>
                                        <span class="block text-xs font-semibold text-gray-400">Estimasi HPP</span>
                                        <span class="text-md font-bold text-gray-800 dark:text-gray-200">{{ fmt(line.unit_cost) }} / pcs</span>
                                    </div>
                                    <div>
                                        <span class="block text-xs font-semibold text-gray-400">Total Nilai</span>
                                        <span class="text-md font-bold text-gray-800 dark:text-gray-200">{{
                                            fmt(line.adjustment_quantity * line.unit_cost)
                                        }}</span>
                                    </div>
                                </div>
                            </div>
                        ...
                        <div class="space-y-4 rounded-2xl border border-gray-100 bg-white p-6 shadow-sm dark:border-gray-800 dark:bg-gray-800">
                            ...
                            <div
                                v-if="!prepOrder || !prepOrder.material_usages || prepOrder.material_usages.length === 0"
                                class="text-gray-450 py-6 text-center text-sm italic"
                            >
                            ...
                            <div v-else class="space-y-4">
                                <div
                                    ...
                                    :key="usage.id"
                                    class="flex items-start justify-between gap-3 border-b border-gray-50 py-2 last:border-b-0 sm:items-center dark:border-gray-800/50"
                                >
                                    <div class="min-w-0" v-if="usage.material_receipt?.material">
                                        <p class="text-sm font-bold text-gray-800 dark:text-gray-200">
                                            {{ usage.material_receipt.material.name }}
                                        </p>
                                        <p class="mt-1 flex flex-wrap items-center gap-1.5 text-xs font-semibold text-gray-400">
                                            <span>Kode: {{ usage.material_receipt.material.code }}</span>
                                            <span class="text-gray-300 dark:text-gray-700">•</span>
                                            <span
                                                class="rounded border border-gray-100 bg-gray-50 px-1.5 py-0.5 font-mono text-[10px] dark:border-gray-800 dark:bg-gray-900"
                                            >

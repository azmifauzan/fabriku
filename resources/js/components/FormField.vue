...
<template>
    <div class="space-y-2">
        ...
        <!-- Select -->
        <select v-else-if="type === 'select'" v-model="inputValue as string | number" :required="required" :disabled="disabled" :class="inputClasses">
            <slot name="options">
        ...
        <div v-else-if="type === 'checkbox'">
            <label class="flex cursor-pointer items-center gap-3">
                <input
                    ...
                    type="checkbox"
                    :disabled="disabled"
                    class="h-5 w-5 flex-shrink-0 rounded border-gray-300 text-indigo-600 focus:ring-2 focus:ring-indigo-500 disabled:cursor-not-allowed dark:border-gray-600 dark:bg-gray-700"
                />
        ...
        <!-- Error Message -->
        <p v-if="error" class="flex items-start gap-1 text-sm text-red-600 dark:text-red-400">

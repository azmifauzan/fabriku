...
const quickSuggestions = ['Berapa penjualan bulan ini?', 'Material apa yang stok rendah?', 'Tunjukkan produksi yang pending'];
...
<template>
    ...
    <button
        v-if="!isOpen"
        @click="toggleChat"
        class="fixed right-6 bottom-6 z-50 flex h-14 w-14 items-center justify-center rounded-full bg-indigo-600 text-white shadow-lg transition-all hover:scale-110 hover:bg-indigo-700 focus:ring-2 focus:ring-indigo-500 focus:ring-offset-2 focus:outline-none"
    ...
    >
        <div
            ...
            :class="[
                ...
                isMinimized ? 'right-6 bottom-6 h-14 w-72' : 'right-6 bottom-6 h-[32rem] w-96 sm:h-[36rem]',
            ]"
        >
            <!-- Header -->
            <div class="flex items-center justify-between border-b border-gray-200 bg-indigo-600 px-4 py-3 dark:border-gray-700">
                <div class="flex items-center gap-3">
                    ...
                    <div>
                        ...
                        <p v-if="!isMinimized && usage" class="text-xs text-indigo-200">{{ remainingMessages }} pesan tersisa hari ini</p>
                    </div>
                </div>
            ...
            <template v-if="!isMinimized">
                ...
                <div ref="messagesContainer" class="flex-1 space-y-4 overflow-y-auto p-4">
                    ...
                    <div v-else-if="!hasMessages" class="flex h-full flex-col items-center justify-center px-4 text-center">
                        <div class="mb-4 flex h-16 w-16 items-center justify-center rounded-full bg-indigo-100 dark:bg-indigo-900/30">
                            <Bot :size="32" class="text-indigo-600 dark:text-indigo-400" />
                        </div>
                        <h4 class="mb-2 text-lg font-semibold text-gray-900 dark:text-white">Halo! 👋</h4>
                        <p class="mb-4 text-sm text-gray-600 dark:text-gray-400">Saya Fabriku Assistant. Ada yang bisa saya bantu?</p>
                        <div class="w-full space-y-2">
                            <button
                                v-for="suggestion in quickSuggestions"
                                :key="suggestion"
                                @click="handleSuggestion(suggestion)"
                                class="w-full rounded-lg bg-gray-100 px-3 py-2 text-left text-sm text-gray-700 transition-colors hover:bg-indigo-100 hover:text-indigo-700 dark:bg-gray-700 dark:text-gray-300 dark:hover:bg-indigo-900/30 dark:hover:text-indigo-300"
                            >
                                {{ suggestion }}
                            </button>
                        </div>
                    ...
                    <template v-else>
                        ...
                        >
                            ...
                            >
                                <div class="prose prose-sm dark:prose-invert max-w-none" v-html="formatContent(message.content)" />
                            </div>
                        </div>
                ...
                <!-- Error Message -->
                <div v-if="error" class="border-t border-red-200 bg-red-50 px-4 py-2 dark:border-red-800 dark:bg-red-900/20">
                ...
                <div class="border-t border-gray-200 p-4 dark:border-gray-700">
                    <form @submit.prevent="sendMessage()" class="flex items-center gap-2">
                        <input
                            ...
                            :disabled="isSending"
                            class="flex-1 rounded-xl border border-gray-300 bg-gray-50 px-4 py-2.5 text-sm text-gray-900 placeholder-gray-500 focus:border-indigo-500 focus:ring-1 focus:ring-indigo-500 focus:outline-none disabled:opacity-50 dark:border-gray-600 dark:bg-gray-700 dark:text-white dark:placeholder-gray-400"
                        />
                        <button
                            type="submit"
                            :disabled="!canSend"
                            class="flex h-10 w-10 items-center justify-center rounded-xl bg-indigo-600 text-white transition-colors hover:bg-indigo-700 focus:ring-2 focus:ring-indigo-500 focus:ring-offset-2 focus:outline-none disabled:cursor-not-allowed disabled:opacity-50"
                        >

...
- `TenantOnboardingService` — shared trial-tenant + admin-user creation (business_category seeding for `retail`/`service`), used by both `RegisterController` (password) and `GoogleAuthController` (OAuth)
...
- **`resources/views/errors/500.blade.php` must gate `$exception->getMessage()` behind `config('app.debug')`** — it renders for every uncaught non-HTTP exception (DB errors, etc.) with the real `$exception`, unlike 400/401/403/404/405/419/422/429/502/503/504 whose messages are always developer-written strings from `abort()`. Printing it unconditionally leaks SQL/stack info to end users in production regardless of `APP_DEBUG`, since Laravel treats a custom `errors::500` view as fully the app's responsibility (no framework-level redaction). Confirmed live via `ExceptionHandler::render()` probe on the prod container even with `APP_DEBUG=false`. All 12 status codes (400/401/403/404/405/419/422/429/500/502/503/504) have Fabriku-themed custom pages in `resources/views/errors/*.blade.php` sharing `errors.layout` (glassmorphic card, dark mode, custom SVG per code) — Laravel resolves them by filename convention, no registration needed; `502`/`504` messages are also gated behind `config('app.debug')` since they can surface upstream/gateway detail.
- **Google OAuth login/register** (`GoogleAuthController` + `TenantOnboardingService`): brand-new Google email → profile held in session (`pending_google_user`) → `Auth/CompleteGoogleRegistration` form collects `business_name`/`business_category` → creates trial tenant same as password register. Existing `users.email` match with no `google_id` → auto-linked (`google_id` set, `email_verified_at` backfilled if null) and logged in; Google already verified the email so no extra confirmation step. Google-only accounts get `Str::random(40)` as their `password` hash (unusable, forces Google-only login) rather than a nullable column — avoids a `doctrine/dbal` dependency for `Schema::table()->change()`. **`User::$fillable` must list every column the onboarding/auth flow mass-assigns** — `email_verified_at` was missing and silently dropped on Google auto-register until a test caught it; there is no exception on a non-fillable mass-assign here, just silent data loss. **`callback()` rejects the whole flow up front unless Google's own `email_verified`/`verified_email` claim is true** — trusting an unverified email claim would let an attacker register a Google account with a victim's Fabriku email and auto-link/take over that account; found by automated security review, covered by `GoogleAuthTest`.
...
- **Vue**: single root element; `<Link>` / `router.visit()` not raw `<a>`; check existing components in `resources/js/components/` (especially `ui/`) before creating new. Number display: import `formatNumber` from `@/lib/utils` for any quantity/stock/count interpolation (`{{ formatNumber(item.current_stock) }}`) — never interpolate a raw numeric prop directly, it silently drops the thousand separator once the value crosses 1000. Several pages had accumulated raw `{{ item.current_stock }}`-style interpolations before this was centralized; `formatNumber` accepts `number | string | null | undefined` and an optional `decimals` (default `0`) for fractional quantities like material kg. Product image display: use `ProductThumbnail.vue` (image + fallback icon) and, for pickers, `ProductSelect.vue` (searchable combobox showing `image_url` per row) instead of a native `<select>` — HTML `<option>` cannot render `<img>`. Business category icons come from the shared map `resources/js/lib/businessCategoryIcons.ts`, not inline emoji or per-page lucide imports — add new categories there so Register/CompleteGoogleRegistration stay in sync. Every public (non-tenant, non-admin) page must render `<SeoHead>` (`resources/js/components/SeoHead.vue`) instead of a raw `<Head>` — it's the single place description/canonical/OG/`noindex`/JSON-LD are emitted; `canonical` should come from the backend (`url()->current()` or `url(...)`, never `window.location`) so it's correct with the SSR bundle. Auth pages (`/login`, `/register`, Google registration) pass `:noindex="true"`. See `docs/seo-audit.md`.
...
- `docs/seo-audit.md` — GSC audit findings (indexing/sitemap/canonical/meta gaps) and the fixes applied for each.
- `docs/seo-keyword-strategy.md` — long-tail keyword research and blog content roadmap for organic growth.

...
### ✅ RESOLVED — Auto-generated tenant-scoped code buntu permanen setelah soft-delete
Ditemukan saat audit lanjutan bug `receipt_number` (lihat entri di bawah): forensic sweep prod untuk 4 tabel lain (`sales_orders`, `production_orders`, `inventory_items`, `inventory_locations`) yang sebelumnya kena bug unique-global-vs-tenant-scoped ternyata tidak punya orphan-row risk (semua create-nya transactional/atomic), tapi nemu bug lain: generator nomor/kode (`SalesOrder::generateOrderNumber()`, `ProductionOrder::booted()`, `InventoryItem::generateSku()`, `InventoryLocation::generateCode()`) query default Eloquent yang otomatis exclude row soft-deleted, padahal unique index `(tenant_id, kolom)` tidak peduli `deleted_at`. Soft-delete row yang lagi pegang nomor tertinggi → generator hitung ulang nomor yang sama persis selamanya → tabrakan permanen dengan row yang di-trash tapi masih fisik ada di tabel, dalam SATU tenant saja (tidak butuh tenant lain). Cek prod: 0 tenant kena saat ini (fix ini preventif, bukan remediasi). Fix: tambah `->withTrashed()` di keempat query generator + catch `UniqueConstraintViolationException` di `ProductionOrderController`/`InventoryLocationController` (SalesOrderController/InventoryItemController sudah punya). Test: `tests/Feature/SalesOrderTest.php`, `tests/Feature/ProductionOrderTest.php`, `tests/Feature/InventoryLocationTest.php`, `tests/Unit/InventoryItemSkuTest.php`.

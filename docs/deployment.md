...
- **Credential SSH server** di `.env` lokal (file ini sudah di `.gitignore`, jangan commit). Tambahkan:
  ```bash
  DEPLOY_SSH_HOST=xxx.xxx.xxx.xxx
  DEPLOY_SSH_USER=ubuntu
  DEPLOY_SSH_KEY_PATH=~/.ssh/id_rsa_fabriku
  DEPLOY_SSH_PORT=22
  ```
...
ssh -i "${DEPLOY_SSH_KEY_PATH}" -p "${DEPLOY_SSH_PORT:-22}" "${DEPLOY_SSH_USER}@${DEPLOY_SSH_HOST}"
```
...
# Ganti baris "image: azmifauzan/fabriku:..." dengan tag baru
sed -i "s|image: azmifauzan/fabriku:.*|image: azmifauzan/fabriku:${TAG}|" docker-compose.yml
...
```bash
docker compose ps
docker inspect --format '{{.Config.Image}}' fabriku
```
...
```bash
docker compose exec app php artisan migrate:status | grep -i "pending\|no" 
```
...
# Hit endpoint dari server
curl -sf -o /dev/null -w "%{http_code}\n" http://localhost/

# Hit dari luar (ganti domain sesuai APP_URL)
curl -sf -o /dev/null -w "%{http_code}\n" https://app.fabriku.my.id/
```

Aplikasi dianggap **aman** bila:
- `docker compose ps` → status `Up (healthy)`.
- `curl` ke `/` return `200`.
- `storage/logs/laravel.log` tidak ada `ERROR`/exception baru sejak deploy.
- Queue worker & scheduler (Supervisor di dalam container) tetap jalan — cek `docker compose exec app supervisorctl status`.

### Rollback

Jika ada masalah, rollback dengan mengganti `image:` ke tag sebelumnya (lihat riwayat di Docker Hub / `docker images` lokal), lalu ulangi langkah 5. Jika migration baru menyebabkan masalah dan butuh rollback schema, jalankan `docker compose exec app php artisan migrate:rollback --force` sebelum downgrade image.

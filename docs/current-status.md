...
| Auth tenant — Google OAuth (login/register) | aktif | tombol "Lanjutkan dengan Google" di atas form Login/Register (`GoogleAuthButton.vue`); email baru → form lengkapi bisnis (`Auth/CompleteGoogleRegistration.vue`) lalu buat trial tenant; email existing tanpa `google_id` → auto-link; callback ditolak (redirect ke `/login` + error) bila Google melaporkan email belum terverifikasi (`email_verified`/`verified_email` claim) — cegah account takeover via email tak terverifikasi; lihat `GoogleAuthController` + `TenantOnboardingService` |
| Halaman legal (Privasi, Syarat & Ketentuan) | aktif | `/privasi`, `/syarat-ketentuan` — static Inertia pages (`Legal/Privacy.vue`, `Legal/Terms.vue`), layout bersama `LegalLayout.vue`; linked dari footer landing page, footer app-shell (tenant), dan halaman login/register/admin-login |
...
| Inventory items | aktif | sumber: production / opening_balance / purchase / return; SKU auto-generate per kategori; **multi-rack split**: create/edit form bisa alokasi stock ke beberapa rak sekaligus (`locations[]`, N `inventory_items` row terpisah per rak, tanpa pivot table); split stock item existing lewat "Pindah/Split Stock" (`POST items/{item}/transfer` → `InventoryService::transferStock()`, hanya `available_stock` yang bisa dipindah, `reserved_quantity` tetap di row asal, row asal soft-delete bila kosong total) — tombol tersedia di Show page dan di Edit form (`Form.vue`, embed `TransferStockModal`); **merge item satu rak**: tombol "Gabung Item" di Show page (`MergeStockModal.vue`, hanya muncul bila ada kandidat) → `POST items/{item}/merge` → `InventoryService::mergeStock()` — pindah seluruh `current_quantity` sumber ke tujuan lalu soft-delete sumber; keduanya harus `reserved_quantity = 0` dan cocok persis di `InventoryService::MERGE_COMPATIBILITY_FIELDS` (location, product_name/code, category, production_order, source_type, unit_cost, selling_price, quality_grade, status, expired_date); daftar kandidat di `InventoryItemController::show()` pakai konstanta yang sama supaya tidak menyarankan item yang nanti ditolak service |
| Inventory item categories | aktif | klasifikasi item tambahan |
| Stock adjustments | aktif | tipe: opening_balance, correction, damage, lost, found, transfer, merge, dst |
| Customers | aktif | CRUD |
| Sales orders | aktif | line items; full status lifecycle `draft→confirmed→processing→shipped→completed`/`cancelled` via state machine `SalesOrder::transitionMap()` + modal Update Status (`PATCH sales-orders/{id}/update-status`); edit form dikunci ke `draft` saja (`canBeEdited()`); cetak invoice + surat jalan; export CSV; **`payment_due_date`**: input jatuh tempo di form, badge Overdue di Index/Show, filter `?payment_status=overdue`; **`shipping_cost`**: input ongkir aktif, masuk `total_amount`; **`invoice_number`**: auto-generate `INV/YYYY/MM/NNNN` saat SO keluar draft (saving hook), manual override tetap bisa; status form dikunci read-only (UI sebelumnya menyesatkan); **foto produk di form Create/Edit**: picker produk diganti dari native `<select>` ke combobox custom `ProductSelect.vue` (searchable, tiap baris list tampil thumbnail dari `inventory_items.image_url`) — native `<select>` tidak bisa render `<img>` di dalam `<option>`; `ProductThumbnail.vue` dipakai juga di Quick Checkout |
...
`config/business.php` — 7 kategori aktif: `garment`, `food`, `craft`, `cosmetic`, `retail`, `homemade`, `service`. Default: `garment`. Setiap tenant memilih satu kategori saat register. Terminologi UI (Pattern/Resep, Cutting/Mixing, dll) di-resolve dari `Tenant::getTerminology($key)`. Icon kategori (dulu emoji mentah dari backend) sekarang di-resolve dari `resources/js/lib/businessCategoryIcons.ts` (map `business_category` → komponen lucide), dipakai di picker kategori `Register.vue` dan `CompleteGoogleRegistration.vue` — kategori baru wajib ditambahkan di sini juga. Daftar demo credentials di `Login.vue` pakai import lucide terpisah (bukan map ini) karena ikonnya per tenant demo spesifik, bukan per kategori generik.
